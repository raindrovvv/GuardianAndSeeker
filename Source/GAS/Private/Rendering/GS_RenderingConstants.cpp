...
namespace GS_Rendering
{
    bool IsRTSMode(const UObject* WorldContext)
    {
        ...
        if (FApp::CanEverRender() == false) return false;
    ...
    {
        if (!WorldContext) return NET_UPDATE_FREQ_MIN;

        UWorld* World = WorldContext->GetWorld();
        if (!World) return NET_UPDATE_FREQ_MIN;
        ...
        if (!bAnyPlayerNear) return NET_UPDATE_FREQ_FAR;
